...
<html>
    ...
    <body class="skin-blue">
        ...
        <div class="wrapper row-offcanvas row-offcanvas-left">
            ...
            <aside class="right-side">
				...
				<?php if(isset($_REQUEST["stat"])) {
					if($_REQUEST["stat"]=="success") {?>
						<div class="box-body">
							<div class="alert alert-danger alert-dismissable">
								<i class="fa fa-ban"></i>
								<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
								<b>Alert!</b> Expense Successfully Saved!
							</div>
						</div>
				<?php }
				if($_REQUEST["stat"]=="updated") {?>
						<div class="box-body">
							<div class="alert alert-danger alert-dismissable">
								<i class="fa fa-ban"></i>
								<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
								<b>Alert!</b> Expense Successfully Updated!
							</div>
						</div>
				<?php }
				} ?>
                                ...
                                <div class="box-body">  
									<form action="addupdexpense_action.php" method="POST" >
										...
										<div class="form-group row">
											...
											<div class="col-xs-6">
												<div class="input-group">
													...
													<input class="form-control" id="transDate" name="transDate" data-inputmask="'alias': 'mm/dd/yyyy'" data-mask="" type="text" placeholder="mm/dd/yyyy" value="<?php echo $transDate;?>">
												</div>		
											</div>		
										</div>											 
										...
										<div class="form-group">
											...
											<input type="text" class="form-control" id="code" name="code" value="<?php echo $expenseCode;?>" readonly required>
										</div>
										<div class="form-group">
										<label for="Head">Head </label>											
											<textarea class="form-control" id="description" name="description" rows="1" required readonly ><?php echo $expenseHead;?></textarea>
										</div>
										<div class="form-group">
       ...
       <input type="number" required id="amount" name="amount" min="0" step="0" data-number-to-fixed="2" data-number-stepfactor="100" class="form-control currency" value="<?php echo $amount;?>" />
									...
									<table id="table-head" class="table table-bordered table-striped">
										...
										?>
											<tr>
												...
												<td>
												<a href="#" class="add-expense" id="<?php echo $row["expensecode"]; ?>"><i class="fa fa-plus-square" data-toggle="tooltip" data-placement="top" title="" data-original-title="Add"></i></a>	
												</td>																													
											</tr>
										<?php 	
										}
										...
										<thead>
											<tr>
												...
												<th width="13%">Amount</th>
												 <th width="8%"><i class="fa fa-cogs" aria-hidden="true"></i></th>
											</tr>
										</thead>
										<tbody>
										<?php													
										$expenseServiceObj = new ExpenseService();													
										$results=$expenseServiceObj->getExpenseEnteredToday('DATALIST',date('Y-m-d'),date('Y-m-d'));
										while($row=mysqli_fetch_array($results)){
										?>
											<tr>																	
												<td><?php echo $row["expenseid"]; ?></td>																		
												<td><?php echo date("m-d-Y",strtotime($row["transdate"])); ?></td>																
												<td><?php echo $row["expensecode"]; ?></td>																
												<td class="narration-list" id="narration<?php echo $row["expensecode"]; ?>"><?php echo $row["narration"]; ?></td>																
												<td class="amount-list" id="amount<?php echo $row["expensecode"]; ?>" align="right"><?php echo moneyDisplayFormat($row["amount"],"0.00"); ?></td>
        ...
        <script src="js/adminlte/demox.js" type="text/javascript"></script>  
        ...
        <!-- AdminLTE App -->
            ...
            $(function() {				
					$('#btn-Save').click(function(e){						
						...
						//var transDate = transDate.split("/").reverse().join("-");						
						amt=$("#amount").val();	
						narr=$("#narration").val();
						if(code==""){
						...
						if(new Date(transDate)>new Date()){
							alert("Please Enter Valid Date!"+transDate);
							$("#transDate").focus();
							return false;
						}	 
						if(narr==""){
				...
				format: 'mm/dd/yyyy', endDate: '<?php echo date('m-d-Y'); ?>'
				});
