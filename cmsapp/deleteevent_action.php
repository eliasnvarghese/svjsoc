...
if(!isset($eventId)){
	header('location:listofevents.php');  
	exit();
}
try{
	if(!$eventServiceObj->isEventIdExists($eventId)){
		header('location:listofevents.php?eventId='.$eventId.'&stat=fail');  
		exit();
	}
	...
	header('location:listofevents.php?eventId='.$eventId.'&stat=deleted');  
}catch(Exception $e){
	header('location:listofevents.php?eventId='.$eventId.'&stat=fail');  
	exit();
}
