...
class EventService
{
	...
	public function isEventExists($eventId,$fromdate,$todate) {
		...
		$todate=convertFromUserDateToYmd($todate);
	 	$cnt=0;
		try{
 			$query = "select count(eventid) as cnt from events_t where deleted = 0 and eventid!= ".$eventId
						. " and ("
	...
	public function getAllTopEventList() {
		$selectString="eventid,eventname,eventdetails,highlights,fromdate,todate,deleted,createdon,createdby";
	 	try{
 			...
 			$query = "select ".$selectString ." from events_t where deleted = 0 " . $orderby . $limit;
		...
		$selectString="eventid,eventname,eventdetails,highlights,fromdate,todate,deleted,createdon,createdby";
			...
			if($searchString!="")
 			...
 			if($resultType=='DATACOUNT')
 				$query = "select count(eventid) as cnt from events_t  ".$searchString;
 			else
 				$query = "select ".$selectString ." from events_t ".$searchString . $orderby . $limit;
	...
	public function deleteEvent($eventid)
	{
		$stat=0;
		try{
			 $query="update events_t set deleted = 1  where eventid = ".quote($eventid);
