...
<?php
	...
	if($userAccount!=null){
		...
		header('location:addincome.php?uid='.$uId.'&stat=failed');
		exit();
	}	
	if($formaction=="add"){
		$rectId = $paymentServiceObj->add($uId, $rectdate, $categorycode, $rectdetls, $rectamount, $createdby);
		...
		if($rectId>0)
			header('location:addincome.php?uid='.$uId.'&stat=saved');
		else
			header('location:addincome.php?uid='.$uId.'&stat=savfailed');
		exit();
	}
	else if($formaction=="update"){
		$stat = $paymentServiceObj->update($uId,$rectId, $rectdate,  $categorycode, $rectdetls, $rectamount);
		if($stat>0)
			header('location:addincome.php?uid='.$uId.'&stat=updated');
		else
			header('location:addincome.php?uid='.$uId.'&stat=updfailed');	
		exit();
	}
	else if($formaction=="cancel"){
		$stat = $paymentServiceObj->cancelCashReceipt($uId,$rectId);
		if($stat>0)
			header('location:addincome.php?uid='.$uId.'&stat=cancelled');
		else
			header('location:addincome.php?uid='.$uId.'&stat=canfailed');
		exit();
	}
	else{
		header('location:addincome.php?uid='.$uId.'&stat=new');
	}
