...
if($_FILES['PhotoPath']['type'] == 'image/pjpeg' ||
	...
	if($stat==1)
	{
		//$error_array["UserId"]="*Add User faild e-mail Id  must be unique";
		$_SESSION["adduserError"]="*Add User failed e-mail Id  must be unique";
		header("Location:addadminuser.php?stat=user name exists");	
	}
	else
	{		
		$userService_obj->addUser($userAccount_obj);
		if($target_path!="")
		{
