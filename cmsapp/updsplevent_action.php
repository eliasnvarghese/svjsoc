...
{
	header('location:editsplevent.php?eventId='.$eventId.'&stat=fail&err=Invalid Date Range!');  
	exit();
}
try{
	if($eventServiceObj->isEventExists($eventId,$eventObj->getFromDate(),$eventObj->getToDate()))
	{
		header('location:editsplevent.php?eventId='.$eventId.'&stat=fail');  
		exit();
	}
...
if($eventId >0){
	...
	header('location:editsplevent.php?eventId='.$eventId.'&stat=success');
}
else
	header('location:editsplevent.php?eventId='.$eventId.'&stat=fail');  
?>
